...
switch ($route) {
    ...
    case 'zinipay/create':
        ...
        if (empty($customerEmail) || strpos($customerEmail, '@') === false) {
            $customerEmail = "user_{$userId}@fahiminternetbd.com";
        }
        ...
        if ($settings) {
            ...
            if (!empty($settings['zinipayDomain'])) $zinipayDomain = $settings['zinipayDomain'];
        }

        // Build Invoice ID & domain variants
        $invoiceId = 'ZINI_' . time() . strtoupper(substr(uniqid(), -5));
        $origin = isset($_SERVER['HTTP_ORIGIN']) ? $_SERVER['HTTP_ORIGIN'] : 'https://fahiminternetbd.com';

        $candidateDomains = [];
        if ($zinipayDomain) $candidateDomains[] = $zinipayDomain;
        $candidateDomains[] = $origin;
        $candidateDomains[] = 'https://fahiminternetbd.com';
        $candidateDomains[] = 'https://fahim-internet.web.app';
        ...
        // Try candidate domains to hit ZiNiPay API
        foreach ($uniqueDomains as $i => $domain) {
            $domainInvoiceId = "{$invoiceId}_" . ($i + 1);
            $successUrl = "{$domain}/#/deposit?zinistatus=success&invoiceId={$domainInvoiceId}";
            $cancelUrl = "{$domain}/#/deposit?zinistatus=cancel&invoiceId={$domainInvoiceId}";

            $payload = [
                'amount' => $amount,
                'invoiceId' => $domainInvoiceId,
                'invoice_id' => $domainInvoiceId,
                'order_id' => $domainInvoiceId,
    ...
    case 'zinipay/verify':
        ...
        if ($pp && isset($pp['status']) && $pp['status'] === 'completed') {
            echo json_encode(['success' => true, 'status' => 'completed', 'amount' => $amountPaid]);
            break;
        }
